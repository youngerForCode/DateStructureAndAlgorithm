...
/*@version 2.1
 * 程序效果：可以实现一维二次函数的最大值搜索，大概率能找出三维二次函数最优解，解决了处理交叉和变异越界问题
 *         可以根据根据basicV和VScope自适应改变选择交叉范围
 *         加入了离线性能函数和在线性能函数
 * 
 * 多级降压收集极最佳工作（暂定三级）
 * 降压范围：0<voltage<10k 采用二进制编码，收集极的三级降压系统的电位分别为整形的V1，V2，V3。
 * @author stephenson 2016-12-26
 */
...
class Individual {
	 ...
	 * 这里把电压的最小值作为一个静态常量
	 * 把电压可以变化的范围作为变异交叉的对象*/
	public static int BASICV1 = 30000; 
	public static int BASICV2 = 60000; 
	public static int BASICV3 = 90000;
	public static int v1Scope =5000;
	public static int v2Scope =5000;
	public static int v3Scope =5000;
	//染色体长度，交叉变异范围都由他决定
	//三条染色体对应三个电压
	private static final char[] digits ={'0','1'};//这个静态变量必须放在chromelength(要用到它)的前面，参考静态变量赋值问题
	public static int chromeLength1 = bitNumber(BASICV1+v1Scope);
	public static int chromeLength2 = bitNumber(BASICV2+v2Scope);
	public static int chromeLength3 = bitNumber(BASICV3+v3Scope);
	// 个体基因总数，一个个体有三条染色体
...
//		return (BASICV1+v1Scope-v1)*(v1-BASICV1);
//		return (BASICV2+v2Scope-v2)*(v2-BASICV2);
//		return (BASICV3+v3Scope-v3)*(v3-BASICV3);
		return (BASICV1+v1Scope-v1)*(v1-BASICV1)+(BASICV2+v2Scope-v2)*(v2-BASICV2);
	...
	//交叉变异等算子需要操作的二进制位数，也就是染色体的基因位长
	private static int bitNumber(int number){
		
		int bitNum=toBinaryChar(number).length;
		return bitNum;
	}
	//将十进制的整形转为二进制的整形，对应的字符数组。注意是倒序
	private static char[] toBinaryChar(int i){
		return toUnsignedChar(i, 1);
	}
	private static char[] toUnsignedChar(int i,int shift){
        char[] buf = new char[32];//整形的数据长度是32位
        int charPos = 32;//控制数组位子
        int radix = 1 << shift;//基数，决定了转化成的是几进制
        int mask = radix - 1;//取最低位上的数
        do {
...
public class GeneAlgorithm {
	...
	public final int EG = 500;// evolution generation 种群总的进化代
	public int g =0;// 当前的进化代数
	public List<Individual> population;//种群
	public final int PSIZE=40;//种群大小
...
/*		UtilPrintC.d(" after selection:");
		for (Individual i : population) 
			UtilPrintC.d(i.toString());*/
		
		population=crossover_population(population);
/*		UtilPrintC.d(" after crossover:");
		for (Individual i : population) 
			UtilPrintC.d(i.toString());*/
		
		mutate(population);
/*		UtilPrintC.d(" after mutation:");
		for (Individual i : population) 
			UtilPrintC.d(i.toString());		*/
...
//		System.out.printf("g=%-3d",g);
		switch (g) {
		case 1:
			System.out.printf("%-5d %-15.2f",g,maxFitness(population));
			System.out.printf("%-15.2f",performanceOffLine());
			System.out.printf("%-15.2f",performanceOnLine());
			System.out.printf("%-10d\r\n",(int) (2*6250000 - maxFitness(population)));
			break;
		case 10:
			System.out.printf("%-5d %-15.2f",g,maxFitness(population));
			System.out.printf("%-15.2f",performanceOffLine());
...
/*		System.out.printf("g=%-5d,maxFitness=%-15.2f",g,maxFitness(population));
		System.out.printf(",P_offline=%-15.2f",performanceOffLine());
		System.out.printf(",P_online=%-15.2f\r\n",performanceOnLine());*/
	...
	public List<Individual> initPopulation(int size,int v1Scope,int v2Scope,int v3Scope){
        ...
        Random random1=new Random();//math.random()是随机数种子
	...
	public  void mutate(List<Individual> population){
		...
		//其实精英也参加了交叉和变异，因为选择算子是复制适应度较大的四分之一
		...
		for (Individual chromosome : population) {//遍历每一个个体
			...
			for(int i=0 ;i<Individual.chromeLength1;i++){//由染色体长度决定
			...
			//如果没有越界才跟新变异以后的值，如果越界就保持原来的值
			if((tempV+shift)>Individual.BASICV1 && (tempV+shift)<(Individual.BASICV1+Individual.v1Scope))
				chromosome.setVoltage1(tempV+shift);
				
			//电压2
			shift = 0;
		    tempV=chromosome.getVoltage2();//取出电压
		    for(int i=0 ;i<Individual.chromeLength2;i++){//由染色体长度决定
	    		if (random.nextFloat() < mutationRate){
	    			int gene = Math.round(random.nextFloat());//随机产生一个等位基因
	    			int bit = (tempV>>i) & 1;//取出第i位
	    			if(gene != bit){
		    ...
		    if((tempV+shift)>Individual.BASICV2 && (tempV+shift)<(Individual.BASICV2+Individual.v2Scope))
		    	chromosome.setVoltage2(tempV+shift);
		    	
			//电压3
		    shift = 0;
		    tempV=chromosome.getVoltage3();//取出电压
		    for(int i=0 ;i<Individual.chromeLength3;i++){//由染色体长度决定
				if (random.nextFloat() < mutationRate){
					int gene = Math.round(random.nextFloat());//随机产生一个等位基因
					int bit = (tempV>>i) & 1;//取出第i位
					if(gene != bit){
		    ...
		    if((tempV+shift)>Individual.BASICV3 && (tempV+shift)<(Individual.BASICV3+Individual.v3Scope))
		    	chromosome.setVoltage3(tempV+shift);
		//单个个体变异完成   
		    
		} 	
		population.set(0, elitism);//采取精英主义，最优秀的个体不进行交叉,直接遗传个下一代
	}
...
*/
	...
	public Individual crossover(Individual individual1,Individual individual2){
		...
		int result = 0;
		/*对电压1的全部基因位交叉，并不是只对变化范围内交叉
		因为BASICV1=3000=1011 1011 1000 ；可以看到他在低9位也有值，用BASICV1+result并不是真正交叉的结果
		  最大值3500的二进制长度就是i的范围，我们这里的长度已经在Individual中求出*/
		for (int i = 0; i < Individual.chromeLength1; i++) {
        ...
        if(result>Individual.BASICV1 && result<(Individual.BASICV1+Individual.v1Scope) )
        	crossed.setVoltage1(result);//没有越界
        else{//如果越界则随机采用其中一个个体的原电压
        	 result =Math.round(random.nextDouble()) == 0 ? individual1_v:individual2_v;
        	 crossed.setVoltage1(result);
        }
		...
		result = 0;
		for (int i = 0; i < Individual.chromeLength2; i++) {
			if (random.nextDouble() < 0.5) {
				int weight = (int) Math.pow(2, i);
				weight = (weight&individual1_v)==weight? weight:0 ; 
				result += weight;
        ...
        if(result>Individual.BASICV2 && result<(Individual.BASICV2+Individual.v2Scope) )
        	crossed.setVoltage2(result);//没有越界
        else{//如果越界则随机采用其中一个个体的原电压
        	 result =Math.round(random.nextDouble()) == 0 ? individual1_v:individual2_v;
        	 crossed.setVoltage2(result);
        }
		...
		result = 0;
		for (int i = 0; i < Individual.chromeLength3; i++) {
			if (random.nextDouble() < 0.5) {
				int weight = (int) Math.pow(2, i);
				weight = (weight&individual1_v)==weight? weight:0 ; 
				result += weight;
        ...
        if(result>Individual.BASICV3 && result<(Individual.BASICV3+Individual.v3Scope) )
        	crossed.setVoltage3(result);//没有越界
        else{//如果越界则随机采用其中一个个体的原电压
        	 result =Math.round(random.nextDouble()) == 0 ? individual1_v:individual2_v;
        	 crossed.setVoltage3(result);
        }
	...
	public  double avgFitness(List<Individual> population){
		...
		//将除数转化为浮点数。除数如果是整数，就是整除！
		return totalFitness/(population.size()+0.0);
	}
	//种群的最大适应度
	...
	//离线性能函数
	//反应群体最佳个体适应值经过平滑处理过后的变化情况
	//描述个体的进化能力和遗传算法的搜索能力
	public static double sumMaxFitness = 0;
    public double performanceOffLine(){
    	sumMaxFitness += maxFitness(population);
    	return sumMaxFitness/(g + 0.0);
    }
    //在线性能函数
  	//反应群体平均适应值经过平滑处理过后的变化情况
  	//描述群体整体性能和进化能力
  	public static double sumFitness = 0;
      public double performanceOnLine(){
          sumFitness += PSIZE*avgFitness(population);    	  
    	  return sumFitness/(g*PSIZE + 0.0);
      }
	//把种群中的每一个的电压数据输出到文件
	...
	public final double Pmin=0.15;//最小的变异概率
